...
describe('packageAddon', () => {
  it('writes contract-validated release artifacts and sidecars', async () => {
    ...
    try {
      ...
      expect(addonEntries).toContain('validation.report.json')
      expect(addonEntries).not.toContain('packos.report.json')
      ...
      expect(releaseManifest).not.toHaveProperty('packos')
      ...
      const submission = await fs.readFile(result.releaseIndexSubmissionPath ?? '', 'utf8')
      expect(submission).toContain('# Release Index Submission')
      expect(submission).toContain('Repository: knoxhack/ECHO-Release-Index')
      expect(submission).toContain('Entry path: addons/weather_pack.json')
      expect(submission).toContain('Source release: teamnova/weather_pack-addon@v1.0.0')
      expect(submission).toContain('Checksums file: checksums.sha256')
      expect(submission).toContain('Required workflow: .github/workflows/release.yml')
      expect(submission).toContain('weather_pack-1.0.0.echo-addon from teamnova/weather_pack-addon@v1.0.0')
      const draft = JSON.parse(await fs.readFile(result.releaseDraftPath ?? '', 'utf8'))
      expect(draft.assets).toHaveLength(9)
      ...
      expect(draft.assets.map((asset: { name: string }) => asset.name)).toContain('release-index-submission.md')
      expect(draft.body).toContain('release-index-submission.md')
