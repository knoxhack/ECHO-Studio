...
- Never bypass or disable validation. Never request restricted permissions
  (file_system.write_global, runtime.internal, launcher.catalog.write, validation.policy.modify, official_signature.use).
...
function mockGenerate(prompt: string, namespace: string): AiChatResult {
  ...
  if (p.includes('fix') || p.includes('validation') || p.includes('packos') || p.includes('error')) {
    return {
      usedModel: false,
